...
class Product(models.Model):
    ...
    vendor = models.ForeignKey(usermodels.User, on_delete=models.SET_NULL, null=True, blank=True)
...
class Cart(models.Model):
    ...
    user = models.ForeignKey(usermodels.User, on_delete=models.SET_NULL, null=True, blank=True)
...
class Coupon(models.Model):
    vendor = models.ForeignKey(usermodels.User, on_delete=models.SET_NULL, null=True)
...
class Order(models.Model):
    vendors = models.ManyToManyField(usermodels.User, blank=True)
    customer = models.ForeignKey(usermodels.User, on_delete=models.SET_NULL, null=True, related_name="customer", blank=True)
    sub_total = models.DecimalField(decimal_places=2, max_digits=12, default=0.00, null=True, blank=True)
    shipping = models.DecimalField(decimal_places=2, max_digits=12, default=0.00, null=True, blank=True)
    tax = models.DecimalField(decimal_places=2, max_digits=12, default=0.00, null=True, blank=True)
    total = models.DecimalField(decimal_places=2, max_digits=12, default=0.00, null=True, blank=True)
    ...
    initial_total = models.DecimalField(default=0.00, max_digits=12, decimal_places=2, help_text="The original total")
    ...
    def order_items(self):
        return OrderItem.objects.filter(order=self) # Fetching all the items
...
class OrderItem(models.Model):
    ...
    applied_coupom = models.BooleanField(default=False)
    item_id = ShortUUIDField(length=6, max_length=25, alphabet="1234567890")
    vendor = models.ForeignKey(usermodels.User, on_delete=models.SET_NULL, null=True, related_name="vendor_order_item")
    date = models.DateTimeField(default=timezone.now)
...
class Review(models.Model):
    ...
    def __str__(self):
        return f"{self.use.username} review on {self.product.name}"
